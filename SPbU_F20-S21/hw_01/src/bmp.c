...
int padding_size(int width) {
	return width % 4;
}

int make_array(pixel ***data, int w, int h) {
...
void update_info(bmp_file *image, int w, int h) {
...
int crop(bmp_file image, bmp_file *new_image, int x, int y, int w, int h) {
	...
	if(make_array(&(new_image->data), w, h)) {
		return -1;
	}

	for(int i = 0 ; i < h; i++)
		for(int j = 0; j < w; j++)
			new_image->data[i][j] = image.data[y + i][x + j];
